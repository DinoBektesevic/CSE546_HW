...
\begin{enumerate}
    ...
    And the same applies to $\E_\text{test}$ with the caveat that the sum would run over $S_\text{test}$ because the pairs $(x_i,y_i)$ remain iid rvs. 
    ...
    Using the same reasoning as above we can expand, and then collapse back, the following expression:
    \begin{align*}
    \E_\text{test}[\widehat\epsilon_\text{test}(\hat f) = \frac{1}{N_\text{test}} \sum_{(x,y)\in S_\text{test}} \E_\text{train}\left[ (f(x)-y)^2 \right]     
    \end{align*}{}
    ...
    Consider the definition of $\hat f = \argmin_{f\in \mathcal{F}} \widehat\epsilon_\text{train}(f)$ in:
    \begin{align*}
        \E_\text{train} [\hat f] &= \E_\text{train}[\argmin_{f\in \mathcal{F}} \widehat\epsilon_\text{train}(f)] \\
        &\leq \argmin_{f\in \mathcal{F}}\E_{\text{test}}[\widehat\epsilon_{\text{train}}(f)] \\
        &\leq \E_{\text{train}}[\widehat\epsilon_{\text{train}}(\hat f_\text{train})] \\
    \end{align*}{}
    
    where we used the rule that $\E_x[\min_\xi f(x, \xi) \leq \min_\xi \E_x[f(x,\xi)]$
\end{enumerate}
